...
subjectscript=/Shared/tientong_scratch/abcd/code/postfmriprep/subject_scripts_push
...
for i in ${!list[@]} ; do
 ...
 if [ -d ${bidsdir}/sub-${SUBJECT}/ses-${SESSION}/func ] &&
  ...
  if [ 1 -eq `ls ${bidsdir}/sub-${SUBJECT}/ses-${SESSION}/func/*rest*.nii.gz 2>/dev/null | wc -l` ] ; then
    sed -e "s|SUBJECT|${SUBJECT}|g" ${template}/postfMRIPrep_singlerun_wrapperpush_TEMPLATE.sh \
    ...
    sed -e "s|SUBJECT|${SUBJECT}|g" ${template}/postfMRIPrep_multirun_wrapperpush_TEMPLATE.sh \
