...
param_grid = {'C': [0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000], 
              'gamma': ['scale','auto', 0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000],
              'kernel': ['linear','rbf', 'poly', 'sigmoid']}

NCV = NestedCV(model=SVC(), params_grid=param_grid, n_jobs = -1, 
               outer_cv=StratifiedKFold(n_splits=5), 
               inner_cv=RepeatedStratifiedKFold(n_splits=5, n_repeats=20, random_state=1), 
               cv_options={'metric':accuracy_score, 
...
print('SVC: ', np.mean(NCV.outer_scores))
